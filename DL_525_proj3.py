...
def run_model(model, x_train, y_train, x_test, y_test_cat, y_test, class_names):
    ...
    history = model.fit(x_train, y_train, epochs=2, batch_size=200, callbacks=[time_callback_train])
...
Small_CNN.add(layers.Conv2D(filters=40, kernel_size=(5, 5), strides=5, padding='valid', activation='relu',
                            input_shape=(28, 28, 1)))
